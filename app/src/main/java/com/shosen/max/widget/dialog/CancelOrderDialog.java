...
public class CancelOrderDialog extends Dialog {
    ...
    public void setText(String[] reasonData) {
        if (rgReason.getChildCount() != reasonData.length) {
            return;
        }

        for (int i = 0; i < rgReason.getChildCount(); i++) {
            RadioButton ra = (RadioButton) rgReason.getChildAt(i);
            ra.setText(reasonData[i]);
        }
    }
}
