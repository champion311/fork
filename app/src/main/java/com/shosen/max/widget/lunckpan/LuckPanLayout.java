...
public class LuckPanLayout extends RelativeLayout {
    ...
    public LuckPanLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        backgroundPaint.setColor(ContextCompat.getColor(getContext(), R.color.pan_outer_color));
        whitePaint.setColor(ContextCompat.getColor(getContext(), R.color.pan_white_dot));
        yellowPaint.setColor(ContextCompat.getColor(getContext(), R.color.pan_yellow_dot));
    ...
    private void drawSmallCircle(boolean FirstYellow) {
        ...
        for (float i = 0; i <= 360; i += 7.5) {
            float x = (float) (pointDistance * Math.sin(Util.change(i)) + (float) CircleX);
            float y = (float) (pointDistance * Math.cos(Util.change(i))) + (float) CircleY;

            if (FirstYellow)
                canvas.drawCircle(x, y, 10, yellowPaint);
            else
                canvas.drawCircle(x, y, 14, whitePaint);
