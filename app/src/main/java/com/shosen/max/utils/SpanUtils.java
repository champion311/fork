...
public class SpanUtils {
    ...
    public static SpannableStringBuilder makeTotalPriceSpan(Context mContext, String header, String text) {
        if (TextUtils.isEmpty(header) || TextUtils.isEmpty(text)) {
            return null;
        }
        String ret = header + text;
        ForegroundColorSpan colorSpan = new ForegroundColorSpan(mContext.getResources().getColor(R.color.red_alert_color));
        SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder(ret);
        spannableStringBuilder.setSpan(colorSpan, header.length(),
                ret.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return spannableStringBuilder;
    }
